...
class WanModel_S2V(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.casual_audio_encoder = CausalAudioEncoder(
            dim=audio_dim, out_dim=self.dim, num_token=num_audio_token, need_global=enable_adain
        )
        ...
        if enable_motioner:
            ...
            self.zip_motion_out = torch.nn.Sequential(
                WanLayerNorm(motioner_dim), zero_module(nn.Linear(motioner_dim, self.dim))
            )
    ...
    def after_transformer_block(self, block_idx, hidden_states):
        if block_idx in self.audio_injector.injected_block_id.keys():
            ...
            if self.use_context_parallel:
                hidden_states = torch.chunk(hidden_states, get_world_size(), dim=1)[get_rank()]
    ...
    def forward(
        ...
        **extra_kwargs,
        ...
        if self.use_context_parallel:
            ...
            self.pre_compute_freqs = torch.chunk(self.pre_compute_freqs, get_world_size(), dim=1)
