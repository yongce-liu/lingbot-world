...
def get_scaled_pose(
    ...
    if rescale_keypoints[4] == None and rescale_keypoints[7] == None:
        ...
        dis_left_hand = (
            (keypoints[4][0] - left_hand[0].x * src_W) ** 2 + (keypoints[4][1] - left_hand[0].y * src_H) ** 2
        ) ** 0.5
        dis_right_hand = (
            (keypoints[7][0] - left_hand[0].x * src_W) ** 2 + (keypoints[7][1] - left_hand[0].y * src_H) ** 2
        ) ** 0.5
...
def retarget_pose(src_skeleton, dst_skeleton, all_src_skeleton, src_skeleton_edit, dst_skeleton_edit, threshold=0.4):
    ...
    rescaled_src_skeleton_ori = rescale_skeleton(
        src_skeleton_ori["height"], src_skeleton_ori["width"], src_skeleton_ori["keypoints_body"], ratio_list
    )
    ...
    if body_flag == "full_body":
        # print('use foot mark.')
        dst_ground_y = (
            max(dst_skeleton["keypoints_body"][10][1], dst_skeleton["keypoints_body"][13][1]) * dst_skeleton["height"]
        )
        ...
        dst_ground_x = (
            (dst_skeleton["keypoints_body"][8][0] + dst_skeleton["keypoints_body"][11][0]) * dst_skeleton["width"] / 2
        )
    ...
    output = write_to_poses(
        all_src_skeleton, none_idx, dst_shape, ratio_list, delta_x, delta_y, rescaled_src_ground_x, body_flag, scale_min
    )
    return output
