        ...
        if device.type == "cuda":
            ...
            providers = [
                (
                    ...
                    {"device_id": device[-1:] if device[-1] in [str(_i) for _i in range(10)] else "0"},
                ),
...
class Yolo(SimpleOnnxInference):
    ...
    def forward(self, img, shape_raw, **kwargs):
        ...
        person_results = [
            [{"bbox": np.array([0.0, 0.0, 1.0 * shape_raw[i][1], 1.0 * shape_raw[i][0], -1]), "track_id": -1}]
...
class ViTPose(SimpleOnnxInference):
    ...
    def forward(self, img, center, scale, **kwargs):
        ...
        points, prob = keypoints_from_heatmaps(
            heatmaps=heatmaps, center=center, scale=scale * 200, unbiased=True, use_udp=False
        )
